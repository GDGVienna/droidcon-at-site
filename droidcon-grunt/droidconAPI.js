...
module.exports = function(grunt) {
  ...
  grunt.registerTask('droidconAPI', 'Create JSON in API format', function() {
    var src = 'app/_data/en';
    var trg = 'dist/api';
    // load files from src: speakers/speakers-data.yml, schedule/sessions-data.yml, schedule/schedule-data.yml
    var speakers = grunt.file.readYAML(src + '/speakers/speakers-data.yml');
    var sessions = grunt.file.readYAML(src + '/schedule/sessions-data.yml');
    var schedule = grunt.file.readYAML(src + '/schedule/schedule-data.yml');
    ...
    for (i=0; i<speakers.length; i++) {
      ...
      aSpeaker = {
        ...
        'photo': 'https://droidcon.at/img/people/' + speakers[i].thumbnailUrl
      }
        ...
        for (k=0; k<schedule[i].timeslots[j].sessionIds.length; k++) {
          ...
          if (sessionId in sessionsById) {
            ...
            if ('imageUrl' in sessionsById[sessionId]) {
              aSession.photo = 'https://droidcon.at/img/' + sessionsById[sessionId].imageUrl;
            }
            // get the duration
    ...
    grunt.file.write(trg + '/v1/speakers', JSON.stringify(speakersOut));
    grunt.file.write(trg + '/v1/sessions', JSON.stringify(sessionsOut));
  });
};
